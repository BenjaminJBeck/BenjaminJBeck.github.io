...
function addResizeBars(el) {
  ...
  const width = (rect.width / canvasRect.width) * 100;
  const height = (rect.height / canvasRect.height) * 100;
  
  selectionOverlay.style.left = left + '%';
  selectionOverlay.style.top = top + '%';
  selectionOverlay.style.width = width + '%';
  selectionOverlay.style.height = height + '%';
  ...
  border.style.border = '2px solid #667eea';
  border.style.pointerEvents = 'none';
  selectionOverlay.appendChild(border);
  ...
  corners.forEach(corner => {
    ...
    handle.style.cursor = corner.includes('top') && corner.includes('left') || corner.includes('bottom') && corner.includes('right') ? 'nw-resize' : 'ne-resize';
    handle.style.pointerEvents = 'auto';
    handle.style.zIndex = '10001';
    
    // Position corners
    if (corner.includes('top')) handle.style.top = '-4px';
    if (corner.includes('bottom')) handle.style.bottom = '-4px';
    if (corner.includes('left')) handle.style.left = '-4px';
    if (corner.includes('right')) handle.style.right = '-4px';
  ...
  sides.forEach(side => {
    ...
    if (side === 'top' || side === 'bottom') {
      ...
      if (side === 'top') handle.style.top = '-3px';
      if (side === 'bottom') handle.style.bottom = '-3px';
    } else {
      ...
      if (side === 'left') handle.style.left = '-3px';
      if (side === 'right') handle.style.right = '-3px';
    }
...
// Replace the mousemove event listener section for "move" interaction mode
document.addEventListener("mousemove", function (e) {
  ...
  if (interactionMode === "move") {
    ...
    // FIXED: Account for canvas zoom when calculating movement
    // The canvas is transformed, so we need to adjust movement by the zoom factor
    const adjustedMovementX = e.movementX / canvasZoom;
    const adjustedMovementY = e.movementY / canvasZoom;
    
    // Get the actual canvas dimensions (1365px width as defined in CSS)
    const actualCanvasWidth = 1365;
    const actualCanvasHeight = window.innerHeight - 100; // Canvas height from CSS calc
    
    // Convert movement to percentage based on actual canvas dimensions
    const offsetX = (adjustedMovementX / actualCanvasWidth) * 100;
    const offsetY = (adjustedMovementY / actualCanvasHeight) * 100;
    ...
    const baseWidth = (el.offsetWidth / actualCanvasWidth) * 100;
    const baseHeight = (el.offsetHeight / actualCanvasHeight) * 100;
    ...
    // Canvas edge snapping using logical bounds (percentage-based)
    // These calculations are now zoom-independent
    
    // Left edge snapping (0%)
    if (Math.abs(visualLeft) < snapThreshold) {
      newLeft = -scaleOffsetX;
    }
    
    // Right edge snapping (100%)
    if (Math.abs(visualRight - 100) < snapThreshold) {
    ...
    // Bottom edge snapping (100%)
    if (Math.abs(visualBottom - 100) < snapThreshold) {
  ...
  // ... rest of the rotation and scale logic remains the same
